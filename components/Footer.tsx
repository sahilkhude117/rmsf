...
const Footer = () => {
  ...
  const contactInfo = [
    {
      text: 'S.No.377/1B/02, Flat No. S-1 Near LIC Office, Gadhinglaj, District- Kolhapur 416502 Maharashtra',
      href: 'https://maps.app.goo.gl/jWvXUBcK7XazLspt6',
    },
    {
      text: 'rmsfoundationindia@gmail.com',
    ...
    <footer id="about" className="bg-gray-300">
      <div className="max-w-screen-lg px-3 sm:px-6 mx-auto text-gray-800 sm:grid md:grid-cols-3 sm:grid-cols-2">
        ...
        <div className="p-5 mt-5">
          <h4 className="text-sm uppercase text-indigo-600 font-bold">
            Support
          </h4>
          {policies.map((item) => (
            <Link
            ...
            >
              {item.name}{' '}
              {item.badge && (
                <span className="text-teal-600 text-xs p-1">{item.badge}</span>
              )}
            </Link>
          ))}
        </div>
        ...
        <div className="p-5 mt-5">
          ...
          {contactInfo.map((item) => (
            <Link
              key={item.text}
              href={item.href}
              className="my-3 block ml-auto hover:underline"
            >
              {item.text}
            </Link>
          ))}
      ...
      <div className="bg-gray-300 pt-2 border-t">
        <div className="max-w-screen-lg mx-auto px-3 py-5 text-center flex flex-col items-center">
          ...
          <p className="text-sm text-gray-800 mt-4">
            &copy; 2024 rmsfoundationindia.org All rights reserved.
          </p>
        </div>
      </div>
    </footer>
